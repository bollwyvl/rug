...
def update(proj, optdict):
	proj.update(recursive='-r' in optdict)


def status(proj, optdict):
	return proj.status(porcelain='-p' in optdict)
...
def main():
	if (len(sys.argv) < 2) or sys.argv[1] not in rug_commands:
		#TODO: write usage
		print 'rug usage'
	else:
		(func, pass_project, optspec,
		long_options, return_stdout) = rug_commands[sys.argv[1]]
		[optlist, args] = getopt.gnu_getopt(
			sys.argv[2:], optspec, long_options)
